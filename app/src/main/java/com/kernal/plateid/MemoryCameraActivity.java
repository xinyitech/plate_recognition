                ...
                if (isCamera) {
                    ...
                    if (nRet != 0) {
                        ...
                        if (number == null || "".equals(number)||"null".equals(number)) {
                            Toast.makeText(MemoryCameraActivity.this, "请重新识别", Toast.LENGTH_SHORT).show();
                            isCamera = false;
                            camera.startPreview();
                        } else {
                            ...
                            intent.setAction("xinyi.recognition.sdk");
...
//		System.out.println("横屏时   左  ："+prp.plateIDCfg.left+"   右  ："+prp.plateIDCfg.right+"     高："+prp.plateIDCfg.top+"    底："+prp.plateIDCfg.bottom);
    ...
    private void setLinearRegion() {
        ...
        prp.plateIDCfg.left = preHeight / 2 - myview.length * preWidth / surfaceView.getHeight() - add;
        prp.plateIDCfg.right = preHeight / 2 + myview.length * preWidth
                / surfaceView.getHeight() + add;
        prp.plateIDCfg.top = preWidth / 2 - myview.length * preWidth / surfaceView.getHeight() - add;
...
//									System.out.println("保存图片-------------");
                                    ...
                                    if (null == number || "".equals(number)||"null".equals(number)) {
                                        Toast.makeText(MemoryCameraActivity.this, "请重新识别", Toast.LENGTH_SHORT).show();                                  isCamera = false;
                                        camera.startPreview();
                                    } else {
                                        intent.putExtra("number", number);
                                        intent.putExtra("color", color);
